...
METHOD='triplet' # NPO, DPO, rmu, WHP, WHP_retrain, circuit_breaker, task_arithmetic, rep_bending, triplet
...
if [ $METHOD == "NPO" ]; then
    ...
    safe_dist_p='norm'
    safe_dist_n='cosine'
    unsafe_dist_p='cosine'
    unsafe_dist_n='cosine'
    encoder=False
    attack=False
    target_layers="-1"
    target_layer_start_idx=20
...
if [ $METHOD == "NPO" ]; then
    ...
    output="${output}_target_layers${target_layers}_${target_layer_start_idx}_num${layers_window_size}_${alpha}_${beta}_${gamma}_${epsilon}_${eta}_${margin_p}_${margin_n}_${max_step}_${safe_dist_p}_${safe_dist_n}_${unsafe_dist_p}_${unsafe_dist_n}_step_${loss_mode}_${alpha_mode}_${attack}_${encoder}"
elif [ $METHOD == "rmu" ]; then
...
accelerate launch --config_file configs/accelerate_zero1.yaml \
    ...
    ${eta:+--loss_eta $eta} \
    ${margin_p:+--loss_margin_p $margin_p} \
    ${margin_n:+--loss_margin_n $margin_n} \
    ${attack:+--loss_attack $attack} \
    ${encoder:+--loss_encoder $encoder} \
    ${safe_dist_p:+--loss_safe_dist_p $safe_dist_p} \
    ${safe_dist_n:+--loss_safe_dist_n $safe_dist_n} \
    ${unsafe_dist_p:+--loss_unsafe_dist_p $unsafe_dist_p} \
    ${unsafe_dist_n:+--loss_unsafe_dist_n $unsafe_dist_n} \
